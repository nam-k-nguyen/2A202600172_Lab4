...
# Sinh viên cần đọc hiểu data để debug test case..
# ===========================================================
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    for item in expenses.split(","):
        if ":" not in item:
            return f"Định dạng expenses sai: '{item}'. Vui lòng sử dụng định dạng 'tên_khoản:số_tiền'."
        name, amount = item.split(":")
        if name.strip() == "" or amount.strip() == "":
            return f"Định dạng expenses sai: '{item}'. Tên khoản và số tiền không được để trống."
        try:
            expenses_dict[name] = int(amount)
        except ValueError:
            return f"Giá trị không hợp lệ cho '{name}': '{amount}'. Vui lòng nhập số nguyên."
    total_expenses = sum(expenses_dict.values())
    remaining_budget = total_budget - total_expenses

    # Format bảng chi tiết
    expense_lines = "\n".join([f"- {name}: {amount:,}đ" for name, amount in expenses_dict.items()])
    result = f"Bảng chi phí:\n{expense_lines}\n---\nTổng chi: {total_expenses:,}đ\nNgân sách: {total_budget:,}đ\nCòn lại: {remaining_budget:,}đ"

    if remaining_budget < 0:
        result += f"\nVượt ngân sách {-remaining_budget:,}đ! Cần điều chỉnh."
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    if city not in HOTELS_DB:
        return f"Không tìm thấy khách sạn tại {city}. Vui lòng chọn thành phố khác."
    ...
    if city in HOTELS_DB:
        ...
        hotels_info = "\n".join(
            [
                f"- {h['name']} ({h['stars']} sao): {h['area']}, "
                f"{h['price_per_night']:,}đ/đêm, Rating: {h['rating']}/5"
                for h in sorted_hotels
            ]
        )

        return f"Khách sạn tại {city} với giá dưới {max_price_per_night:,}đ/đêm:\n{hotels_info}"
...
def search_flights(origin: str, destination: str) -> str:
    ...
    has_flight = (origin, destination) in FLIGHTS_DB
    has_reverse_flight = (destination, origin) in FLIGHTS_DB

    if not has_flight and not has_reverse_flight:
        return f"Không tìm thấy chuyến bay từ {origin} đến {destination}."
        
    if not has_flight and has_reverse_flight:
        flights = FLIGHTS_DB[(destination, origin)]
        flights_info = "\n".join(
            [
                f"- {f['airline']}: {f['departure']} -> {f['arrival']}, "
                f"{f['class'].capitalize()}, {f['price']:,}đ"
                for f in flights
            ]
        )
        return f"Không tìm thấy chuyến bay từ {origin} đến {destination}, nhưng có chuyến từ {destination} đến {origin}. \n{flights_info}"
    ...
    if has_flight:
        ...
        return f"Chuyến bay từ {origin} đến {destination}:\n{flights_info}"
